...
### Site-wide signature touches (live)
- **Navbar**: active page gets a persistent gradient underline (`aria-current="page"`); a 2px gradient reading-progress bar runs along the header's bottom edge once scrolled.
- **Footer**: 4-column brand closer (statement / Work together / Company / CTA) with gradient top hairline — not a thin one-liner.
- **PointerGlow** (`components/Primitives.tsx`): mouse-tracking radial spotlight for any `relative` rounded card. Used on all pricing cards (blue subtle on light, purple stronger on featured dark). Drop `<PointerGlow />` as the first child.
- **Homepage hero**: word-by-word headline reveal (stagger 70ms, blur+rise per word) — its signature alongside the ParticleCanvas.
- **Brand `::selection`** (electric blue) and `:focus-visible` ring, global.
